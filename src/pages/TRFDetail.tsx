...
import { MockAIAssessment, type DemoScenario } from '@/components/demo/MockAIAssessment';
import { 
  ArrowLeft, 
  ChevronDown,
  Clock,
...
export default function TRFDetail() {
  useEffect(() => { tagScreen('ai-trf-detail'); }, []);
  const { id } = useParams();
  const navigate = useNavigate();
  const [searchParams] = useSearchParams();
  const [demoScenario, setDemoScenario] = useState<DemoScenario>("reject");

  const isDemoMode = typeof window !== 'undefined' && (
    localStorage.getItem('demoMode') === 'true' || searchParams.get('demo') === '1'
  );
  ...
  return (
    ...
    >
      ...
      {/* Demo Scenario Toggle (only in demo mode) */}
      {isDemoMode && (
        <div className="flex items-center gap-2 mb-2">
          <span className="text-xs font-medium text-muted-foreground">Demo scenario:</span>
          {(["approve", "escalate", "reject"] as DemoScenario[]).map((s) => (
            <Button
              key={s}
              size="sm"
              variant={demoScenario === s ? "default" : "outline"}
      ...
      {/* AI Assessment (MockAIAssessment) */}
      <MockAIAssessment
        trfId={id || "TRF-1001"}
        demoScenario={isDemoMode ? demoScenario : undefined}
      />
