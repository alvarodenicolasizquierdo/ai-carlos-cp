...
export default function StyleDetail() {
  ...
  const [dppScenario, setDppScenario] = useState<"high" | "medium" | "low">("medium");
  ...
  return (
    <AppLayout>
      ...
      <Card className="mb-6">
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            <CardTitle className="text-base">DPP Readiness Score</CardTitle>
            {isDemoMode && (
              <div className="flex items-center gap-1.5">
                ...
                {(["high", "medium", "low"] as const).map((s) => (
                  <Button
                    key={s}
                    size="sm"
                    variant={dppScenario === s ? "default" : "outline"}
                    className="text-xs h-7 px-2.5 capitalize"
                    onClick={() => setDppScenario(s)}
                  >
                    {s}
                  </Button>
                ))}
        ...
        <CardContent>
          <MockReadinessGauge scenario={isDemoMode ? dppScenario : "medium"} />
        </CardContent>
      </Card>
