...
export const CSVImportDialog = ({ open, onOpenChange, onImport }: CSVImportDialogProps) => {
  ...
  const parseCSV = useCallback((text: string): { products: ParsedProduct[]; errors: string[] } => {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      const code = sanitizeCsvValue(values[columnMap['Product Code']] || '');
      const name = sanitizeCsvValue(values[columnMap['Name']] || '');
      const category = sanitizeCsvValue(values[columnMap['Category']] || '');
      const supplier = sanitizeCsvValue(values[columnMap['Supplier']] || '');
      const statusRaw = sanitizeCsvValue(values[columnMap['Compliance Status']] || 'pending_review');
