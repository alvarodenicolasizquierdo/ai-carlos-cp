...
function DraggableCard({ inspection }: { inspection: RichInspection }) {
  ...
  return (
    ...
    >
      <Card
        className="cursor-grab active:cursor-grabbing hover:shadow-card-hover hover:border-primary/20 transition-all duration-200"
        onClick={handleClick}
      >
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (over && active.id !== over.id) {
      ...
      if (inspection && statusColumns.some(col => col.id === newStatus)) {
        tagEvent('inspection_status_changed', `${inspection.inspectionNumber}:${newStatus}`);
