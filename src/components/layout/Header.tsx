    ...
    <header className="h-14 md:h-16 bg-background/80 backdrop-blur-lg border-b border-border sticky top-0 z-40 hidden md:block">
      <div className="h-full flex items-center justify-between px-4 md:px-6">
        ...
        <div className="min-w-0 flex-1 mr-4">
          ...
          {subtitle && (
            <p className="text-xs md:text-sm text-muted-foreground leading-relaxed truncate">{subtitle}</p>
          )}
        </div>
        
        {/* Demo Mode Toggle + Reset */}
        <div className="shrink-0 mr-3 flex items-center gap-2">
          <DemoModeToggle />
          <DemoResetButton />
        </div>
        ...
        <div className="flex items-center gap-2 md:gap-3 shrink-0">
          ...
          <div
            className={cn(
              "relative hidden sm:block transition-[width] duration-300 ease-in-out",
              searchFocused ? "w-[400px]" : "w-[280px]"
            )}
          >
