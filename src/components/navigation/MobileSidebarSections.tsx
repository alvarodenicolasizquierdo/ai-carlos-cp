...
export function MobileSidebarSections({ isAdmin }: MobileSidebarSectionsProps) {
  ...
  const DEMO_ALLOWED_PATHS = [
    '/', '/styles', '/components', '/care-labelling',
    '/trfs', '/inspections', '/testing-levels',
    '/suppliers', '/analytics', '/gsw', '/support-center',
  ];

  const filterGroup = (group: NavGroup): NavGroup => ({
    ...group,
    items: group.items.filter(item => DEMO_ALLOWED_PATHS.includes(item.path)),
  });

  const showGovernance = !isDemoMode;

  return (
    <div className="space-y-2">
      <MobileSidebarSection 
        title="My Work" 
        items={isDemoMode ? getMyWorkItems().filter(i => DEMO_ALLOWED_PATHS.includes(i.path)) : getMyWorkItems()} 
        isAdmin={isAdmin}
      />
      <MobileSidebarSection 
        title="Products" 
        group={isDemoMode ? filterGroup(productsGroup) : productsGroup}
        isAdmin={isAdmin}
      />
      <MobileSidebarSection 
        title="Testing" 
        group={isDemoMode ? filterGroup(testingGroup) : testingGroup}
        isAdmin={isAdmin}
      />
      <MobileSidebarSection 
        title="Partners" 
      ...
      <MobileSidebarSection 
        title="Insights" 
        group={isDemoMode ? filterGroup(insightsGroup) : insightsGroup}
        isAdmin={isAdmin}
      />
      <MobileSidebarSection 
        title="Support" 
